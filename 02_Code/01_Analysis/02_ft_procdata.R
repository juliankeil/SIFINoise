...
for (participant in participants){ # loop participants
    ...
    sd_RT <- c(round(sd(subset(incong_no_noise, id == participant)$RT), digits = 3),
               round(sd(subset(cong_no_noise, id == participant)$RT), digits = 3),
               round(sd(subset(incong_noise, id == participant)$RT), digits = 3),
               round(sd(subset(cong_noise, id == participant)$RT), digits = 3))
    ...
    summ_data <- as.data.frame(rbind( summ_data, cbind(id, congruency, noise, error_rate, median_RT, sd_RT)))
